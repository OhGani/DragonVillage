...
**구현 상태(2026-09-18, M2)**: `messages.ts` 에 M2 범위 구현 — PlayerMove·PlayersState·BlockChangeReq/Changed/Rejected·**BlockBatch(0x13, 서버 액체 흐름 묶음)**·ChunkData·Ping/Pong 과 JSON hello/join/create/welcome/ready/playerJoined/playerLeft/error. 아래 표와 다른 점: **블록은 숫자 대신 문자열 id**(결정 #39, `u8 len + UTF-8`), **ChunkData(0x20)는 diff 목록 대신 `serialize.ts` 청크 blob 통째**(결정 #60), yaw/pitch 는 f32. **M3(2026-09-19)**: 원정은 세계 전환·정산이 드물어 JSON 으로 — C→S `startExpedition{expedition}`·`returnHome`, S→C `worldEnter{kind, expedition{seed,startedAt,…}, spawn, players, chunkCount}` → ChunkData… → `ready`, `expeditionResult{items, late, keepRatio}`, `expeditionState`(마을 사람에게, welcome 에도 실림). 바이너리는 1Hz `ExpeditionTimer 0x30 (u16 elapsedSec, u16 durationSec, u8 phase 0낮/1저녁/2밤)` 만. 아래 표의 0x21·0x22·0x31·0x32 는 쓰지 않는다. **M4(2026-09-19)**: 가방 — welcome 에 `inventory`(37칸), S→C `InvSlots 0x14`(u8 n, 반복 u8 slot·str item·u8 count), C→S `InvMove 0x15`(u8 from,to,count)·`InvDrop 0x16`(u8 slot,count), JSON `craft{recipe}`·`brew{bottles[],ingredient}`. 채팅 `Emote 0x40`(u8 idx, u8 kind 0 이모지/1 문구, u8 id) 양방향. 거절 사유 9 `NO_ITEM`. **M5-1(2026-09-19)**: 계정 = 닉네임 + PIN(#63). C→S `setPin{pin}`(마을 안에서), `resume{nick,pin}`(hello 뒤·join 전) → S→C `resumed{token}`(클라가 토큰을 바꿔 저장하고 다시 join), welcome `needPin`. 오류 `NICK_TAKEN`·`BAD_PIN`·`NO_PIN`·`PIN_LOCKED`. **상자(2026-09-22, #84)**: C→S `openChest{x,y,z}`·`chestMove{x,y,z,from,to,count}`(from·to 는 0~상자칸−1 상자, 그다음 가방 37칸), S→C `chest{x,y,z,slots}`(x·y·z 는 큰 상자면 대표 칸, slots 27 또는 54). 오류 NO_CHEST·TOO_FAR. 큰 상자로 합쳐지면 두 칸 모두 `BlockChanged` 로 변형 id(`chest@e`/`chest@w`)를 보낸다. **문 경첩(2026-09-22, #83)**: 변형 id 에 경첩이 붙는다 — 왼쪽은 예전과 같고(`oak_door@n^>`) 오른쪽은 끝에 `r`(`oak_door@n^>r`). 놓을 때 클라가 벽을 보고 고르고, 클라가 문 아이템 id 를 그냥 보내면 서버가 같은 규칙으로 고른다. **문(2026-09-19, #71)**: 놓기·열고 닫기·부수기 모두 기존 `BlockChangeReq` — 놓기는 `oak_door`(서버가 보는 방향 변형으로 바꿈) 또는 `oak_door@n`, 열고 닫기는 조준한 반쪽의 열림만 뒤집힌 변형 id(`oak_door@n^>`), 서버는 다른 반쪽도 바꿔 `BlockChanged` 두 개를 보낸다. **M5-2(2026-09-19)**: 가족 — C→S `linkFamily{code,pin}`(마을 안에서, 가족 코드 6자리 + 내 PIN) → S→C `familyLinked{code}`, welcome `family`(연결된 가족 코드 또는 null). 부모 쪽은 WebSocket 이 아니라 같은 포트의 HTTP `/family`(정적 HTML) + `/api/family/me|signup|login|logout|unlink`(JSON, 쿠키 `dv_parent` HttpOnly 30일). 오류 `NO_FAMILY`(코드 없음)·`BAD_PIN`·`PIN_LOCKED`. **M5-3(2026-09-19)**: 할 일·시간은 JSON — welcome `today`(아이의 `TodayCard`: date·baseMin·bonusCap·bonusMin·manualAdj·usedMin·remainingMin·enforced·blocked·todos[{id,title,needsApproval,status}])·`parentOf`(부모 플레이어면 가족 코드), C→S `checkTodo{id}`, S→C `today{card}`(체크·승인·1분 경과·할 일 편집 때), 부모 플레이어에게 S→C `approvalAsk{id,date,child,title}` → C→S `approveTodo{id,date,ok}`. 오류 `NOT_CHILD`·`NO_TODO`·`NOT_TODAY`·`ALREADY`·`NOT_PARENT`. 부모 HTTP 는 `/api/family/me`(children[{nick,online,today,todos}]·pending·parentPlayers·enforced) + `todoAdd|todoUpdate|todoDelete|decide|linkParent|unlinkParent|adjust|noPlay|resetPin`. **M5-4(2026-09-20, #73)**: 제한이 켜져 있을 때만 — join 거절 `BLOCKED_HOURS`·`NO_PLAY_TODAY`·`TIME_UP`(연결 닫힘), S→C `timeUp{reason: noPlay|blocked|over|idle, message}` 뒤 게임 메시지 무시, 클라가 확인을 누르면 스스로 닫고 아니면 60초 후 서버가 close 4002, startExpedition 거절 `NOT_ENOUGH_TIME`. TodayCard 에 noPlayToday·minutesUntilBlocked·nextOpen·adjustments[{min,reason}] 추가. **M5-5(2026-09-20, #74)**: TodayCard 에 lastWeekRate(0~1 또는 null)·weekMessage 추가, `/api/family/me` children[].settlements[{weekStart,rate,bonusCap,approved,expected}] (최근 4주). **곡괭이(2026-09-20, #75)**: `BlockChangeReq 0x10` 끝에 `u8 slot`(든 핫바 칸, 255 = 없음) — 서버가 그 칸의 곡괭이 등급으로 `REJECT.TOOL(3)` 판정. **선물(2026-09-21, #79)**: welcome `gifts`([{id,name,message}]) — 이번 입장에 받은 것. 아이템은 이미 `inventory` 에 들어 있다. **M6-4(2026-09-20, #78)**: 탑승 — C→S `ride{id}`·`dismount`, S→C `mount{idx, riding{id,dragon}}`·`dismount{idx}`(같은 세계 모두, 본인 포함), PlayerInfo 에 `riding`(welcome·worldEnter·playerJoined), PlayerMove flags 에 `FLAG_RIDING 16`. 탄 드래곤은 `nest.dragons` 에서 빠진다. 오류 NOT_ADULT·NO_SADDLE·TOO_FAR·ALREADY_RIDING·NOT_RIDING. 보물 상자 = `InvSlots` 로 가죽 2. **M6-3(2026-09-20, #77)**: 둥지 드래곤 — welcome `nestDragons`([{id,dragon,owner,mine,stage,perch{x,y,z},yaw,fed,growAt}]), S→C `nest{slots, dragons}`(둥지가 바뀔 때마다 마을 모두), C→S `feed{id,item}`(먹이), 내 목록 `dragons` 항목에 fed·growAt. 오류 NO_DRAGON·NOT_BABY·NOT_FOOD·NO_ITEM. 성장은 서버가 5초마다 확인해 `nest`·`dragons`·`XpGained(source 5)`. **M6-2(2026-09-20, #76)**: 드래곤 — welcome `dragons`(내 것 [{id,dragon,stage,slot,hatchedAt}])·`nest`(자리 [{slot,dragon,owner,mine,id}]), C→S `placeEgg{slot,item}`·`hatch{id}`, S→C `dragons{list}`(나에게)·`nest{slots}`(마을 모두), 알 블록은 `BlockChanged`. 오류 NOT_AT_NEST·NO_EGG·BAD_SLOT·SLOT_TAKEN·NEED_LEVEL(문구에 필요/현재 레벨). **M6-1(2026-09-20)**: 경험치 — welcome `xp`(총량), S→C `XpGained 0x52`(u16 amount, u8 source: 0 캐기·1 귀환·2 보물·3 도감·4 부화·5 성장·6 몹, f32 x y z = 구슬 자리)·`XpState 0x53`(u32 total, 귀환 뒤 정정). 레벨·바는 클라가 `shared/rules/xp.ts` 공식으로 계산. 부화 요청(HatchReq)은 M6-2 에서.
