...
- 마일스톤 단위로 진행. **M2 통과 2026-09-19**(아빠 PC + 아들 폰 실접속 확인). **M3 통과 2026-09-19**(아빠 "원정 확인"). 현재: **M4 (가방·조합·채팅)** 착수 2026-09-19 — 블록 유한(결정 #66) + 시작 키트(`data/starter-kit.json`, #67), 가방·제작·양조기·채팅 전부 서버 권위. 공용 규칙·서버·클라(가방 화면 🎒/E·핫바 개수·만들기·양조 탭·채팅 💬/T) **구현 완료, 라이브 배포 2026-09-19**. 남은 것: 4절(아들이 레시피 하나 추가, 채팅 문구 확정)·5절 플레이테스트. `docs/ROADMAP.md` M4 분해 참고. `docs/ROADMAP.md` M3 분해 참고. `docs/ROADMAP.md` M1 분해 참고. M0 는 아들 폰 피드백으로 실질 통과(정식 5분 관찰과 fps 수치는 `PLAYTEST-LOG.md` 에 채우는 중).
