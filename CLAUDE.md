...
- 마일스톤 단위로 진행. **M2 통과 2026-09-19**(아빠 PC + 아들 폰 실접속 확인). 현재: **M3 (원정 루프)** 착수 2026-09-19 — 원정지 데이터 로더·초원 섬 생성기·서버 원정 룸(`server/expedition.ts`, 프로토콜 worldEnter/expeditionResult/ExpeditionTimer) 완료, 다음은 클라 포탈·세계 전환·타이머·정산 창. `docs/ROADMAP.md` M3 분해 참고. `docs/ROADMAP.md` M1 분해 참고. M0 는 아들 폰 피드백으로 실질 통과(정식 5분 관찰과 fps 수치는 `PLAYTEST-LOG.md` 에 채우는 중).
