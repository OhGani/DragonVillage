...
const TEX = {
  ...
  // 횃불 (#82): 입체 막대로 그리므로 7·8열의 아래 10줄(6~15)만 쓴다.
  // 막대(9~15) 위에 불꽃(6~8) — 옆면은 이 띠를 그대로, 윗면은 6~8줄(불꽃)을 본다
  torch: (c) => {
    c.clear();
    for (let y = 9; y < S; y++) {
      const stripe = y % 3 === 0 ? 0.74 : 1; // 나뭇결
      c.set(7, y, 138 * stripe, 104 * stripe, 60 * stripe);
      c.set(8, y, 99 * stripe, 72 * stripe, 41 * stripe);
    }
    c.set(7, 8, 255, 246, 190); // 불꽃 밑동 (가장 밝다)
    c.set(8, 8, 255, 232, 130);
    c.set(7, 7, 255, 214, 60);
    c.set(8, 7, 255, 196, 48);
    c.set(7, 6, 255, 146, 26);
    c.set(8, 6, 255, 128, 18);
  },
  flower: (c) => {
    c.clear();
