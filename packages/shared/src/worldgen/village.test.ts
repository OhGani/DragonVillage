...
/**
 * 세계 전체의 지문. 블록 **문자열 id** 를 해시하므로 아들이 blocks.json 순서를 바꿔도 같다.
 * 생성기를 고치면 이 값이 바뀐다 → 의도한 변경이면 SNAPSHOT 을 갱신하고 VILLAGE_GEN_VERSION 을 올린다.
 */
function fingerprint(world: VoxelWorld, registry: BlockRegistry): string {
