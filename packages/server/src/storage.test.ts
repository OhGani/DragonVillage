...
describe('Storage (SQLite 메모리)', () => {
  it('마을·청크·플레이어 라운드트립', () => {
    ...
    s.savePlayer({ token: 'ab'.repeat(16), village: '482913', nick: '아들', color: 5, x: 1.5, y: 41, z: 2.5, yaw: 0.5, pitch: -0.1, lastSeen: 999, xpTotal: 55 });
    expect(s.getPlayer('ab'.repeat(16))).toMatchObject({ nick: '아들', color: 5, x: 1.5, yaw: 0.5 });
    s.savePlayer({ token: 'ab'.repeat(16), village: '482913', nick: '아들2', color: 6, x: 3, y: 41, z: 4, yaw: 0, pitch: 0, lastSeen: 1000, xpTotal: 60 });
    expect(s.getPlayer('ab'.repeat(16))).toMatchObject({ nick: '아들2', x: 3, xpTotal: 60 });
