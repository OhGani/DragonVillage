...
export class TouchControls implements InputSource {
  ...
  private readonly onStart = (e: TouchEvent) => {
    ...
    for (const t of Array.from(e.changedTouches)) {
      ...
      if ((t.target as Element | null)?.closest?.('.hotbar, .tbtn, .sbtn, .topbar, .overlay, .help-panel, .action-card, .result-panel, .bag-panel, .chat-panel, .side-btns, .time-chip, .today-panel, .approval-card')) continue;
      handled = true;
